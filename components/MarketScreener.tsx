...
const MarketScreener: React.FC<MarketScreenerProps> = ({ onSelectPair }) => {
  ...
  const getSignalColor = (signal: string) => {
    switch (signal) {
      case 'STRONG_BUY':
        return 'text-trade-green bg-green-900/30 border-green-800';
      case 'BUY':
        return 'text-green-400 bg-green-900/20 border-green-800/50';
      case 'STRONG_SELL':
        return 'text-trade-red bg-red-900/30 border-red-800';
      case 'SELL':
    ...
    <div className="flex flex-col h-full bg-gray-950 p-6 overflow-hidden relative">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">
            Real-time technical analysis &amp; volatility tracking
          </p>
          {lastUpdated && (
      ...
      <div className="flex-1 bg-gray-900 rounded-xl border border-gray-800 overflow-hidden flex flex-col shadow-xl">
        ...
        <div className="grid grid-cols-8 bg-gray-850 p-4 border-b border-gray-800 text-xs font-semibold text-gray-400 uppercase tracking-wider">
          <div
            className="cursor-pointer hover:text-white col-span-1"
            onClick={() => handleSort('symbol')}
          >
            Symbol
          </div>
          <div
            className="cursor-pointer hover:text-white text-right col-span-1"
          ...
          <div
            className="cursor-pointer hover:text-white text-center col-span-1"
          ...
          {loading ? (
            ...
            sortedData.map((ticker) => (
              ...
              >
                ...
                <div className="text-right font-mono text-gray-300 col-span-1">
                  {ticker.price}
                </div>
                <div
                  className={`text-right font-mono font-medium col-span-1 ${
                ...
                <div className="flex justify-center col-span-1">
                  {ticker.trend === 'UP' && (
                    <ArrowUpRight className="text-trade-green" size={18} />
                  )}
                  {ticker.trend === 'DOWN' && (
                    <ArrowDownRight className="text-trade-red" size={18} />
                  )}
                  {ticker.trend === 'SIDEWAYS' && (
                    <Minus className="text-gray-500" size={18} />
                  )}
                </div>
                <div className="text-center font-mono text-gray-400 col-span-1">
                  {ticker.rsi}
                </div>
                <div className="flex justify-center col-span-1">
                  <span
                ...
                <div className="flex justify-center col-span-1">
                  <span
                    className={`px-3 py-1 rounded-full text-[10px] font-bold border ${getSignalColor(
                      ticker.signal
                    )}`}
                  >
            ...
            <div className="p-4 border-b border-gray-800 flex justify-between items-center bg-gray-950">
              <div className="flex items-center gap-3">
                <h3 className="text-xl font-bold text-gray-100">
                  {reportTicker.symbol} Report
                </h3>
                <span
                  className={`px-2 py-0.5 rounded text-xs font-bold border ${getSignalColor(
                    reportTicker.signal
                  )}`}
                >
